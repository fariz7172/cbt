...
@section('breadcrumb')
    ...
    <span class="text-gray-300">/</span>
    <span class="text-gray-600 font-medium">Edit Soal</span>
@endsection

@section('content')
<div class="page-header">
    <h1 class="page-title">Edit Soal</h1>
    <p class="page-subtitle">Ubah informasi soal</p>
</div>

<div class="card max-w-4xl">
    <form action="{{ route('guru.bank-soal.update', $soal) }}" method="POST" id="soalForm" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <select name="pelajaran_id" id="pelajaran_id" class="form-select @error('pelajaran_id') border-danger @enderror" required>
                    ...
                    @foreach($pelajarans as $pelajaran)
                        <option value="{{ $pelajaran->id }}" {{ old('pelajaran_id', $soal->pelajaran_id) == $pelajaran->id ? 'selected' : '' }}>
                            {{ $pelajaran->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="tingkat_kelas" id="tingkat_kelas" class="form-select @error('tingkat_kelas') border-danger @enderror" required>
                    ...
                    @foreach($tingkatKelas as $tingkat)
                        <option value="{{ $tingkat }}" {{ old('tingkat_kelas', $soal->tingkat_kelas) == $tingkat ? 'selected' : '' }}>Kelas {{ $tingkat }}</option>
                    @endforeach
                </select>
                @error('tingkat_kelas')
                    <p class="text-danger text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <select name="tipe" id="tipe" class="form-select @error('tipe') border-danger @enderror" required>
                    ...
                    <option value="pilihan_ganda" {{ old('tipe', $soal->tipe) == 'pilihan_ganda' ? 'selected' : '' }}>Pilihan Ganda</option>
        ...
        <div class="mb-6">
            ...
            <textarea name="pertanyaan" id="pertanyaan" rows="4" 
                      ...
                      placeholder="Tulis pertanyaan di sini..." required>{{ old('pertanyaan', $soal->pertanyaan) }}</textarea>
            @error('pertanyaan')
                <p class="text-danger text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            @if($soal->gambar)
                <div class="mb-3 p-3 bg-secondary-100 rounded-xl" id="currentImage">
                    <p class="text-sm text-gray-500 mb-2">Gambar saat ini:</p>
                    <img src="{{ asset('uploads/' . $soal->gambar) }}" alt="Gambar Soal" class="max-w-xs rounded-lg shadow mb-2">
                    <label class="flex items-center gap-2 text-sm text-danger cursor-pointer">
                        <input type="checkbox" name="hapus_gambar" value="1" class="form-checkbox">
            ...
            <div class="space-y-3">
                ...
                @foreach(['A', 'B', 'C', 'D', 'E'] as $index => $label)
                    <div class="flex items-center gap-3">
                        ...
                        <input type="text" name="opsi[]" class="form-input flex-1" placeholder="Opsi {{ $label }}" value="{{ $opsiValues[$index] ?? '' }}">
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div id="benarSalahContainer" class="mb-6 hidden">
            ...
            <div class="flex gap-4">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="opsi_bs" value="Benar" class="form-radio" {{ strtolower($soal->kunci_jawaban) === 'benar' ? 'checked' : '' }}>
                    <span>Benar</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="opsi_bs" value="Salah" class="form-radio" {{ strtolower($soal->kunci_jawaban) === 'salah' ? 'checked' : '' }}>
                    <span>Salah</span>
                </label>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="kunci_jawaban" id="kunci_jawaban" 
                       ...
                       placeholder="Masukkan kunci jawaban" value="{{ old('kunci_jawaban', $soal->kunci_jawaban) }}" required>
            ...
            <div>
                ...
                <input type="number" name="poin" id="poin" min="1" 
                       ...
                       placeholder="Masukkan poin" value="{{ old('poin', $soal->poin) }}" required>
                @error('poin')
                    <p class="text-danger text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
