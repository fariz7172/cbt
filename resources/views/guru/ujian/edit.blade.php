...
@section('breadcrumb')
    ...
    <span class="text-gray-300">/</span>
    <span class="text-gray-600 font-medium">Edit Ujian</span>
@endsection

@section('content')
<div class="page-header">
    <h1 class="page-title">Edit Ujian</h1>
    <p class="page-subtitle">Ubah informasi ujian</p>
</div>
    ...
    <form action="{{ route('guru.ujian.update', $ujian) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <select name="pelajaran_id" id="pelajaran_id" class="form-select @error('pelajaran_id') border-danger @enderror" required>
                    ...
                    @foreach($pelajarans as $pelajaran)
                        <option value="{{ $pelajaran->id }}" {{ old('pelajaran_id', $ujian->pelajaran_id) == $pelajaran->id ? 'selected' : '' }}>
                            {{ $pelajaran->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="rombel_id" id="rombel_id" class="form-select @error('rombel_id') border-danger @enderror" required>
                    ...
                    @foreach($rombels as $rombel)
                        <option value="{{ $rombel->id }}" {{ old('rombel_id', $ujian->rombel_id) == $rombel->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="jenis" id="jenis" class="form-select @error('jenis') border-danger @enderror" required>
                    ...
                    <option value="ulangan_harian" {{ old('jenis', $ujian->jenis) == 'ulangan_harian' ? 'selected' : '' }}>Ulangan Harian</option>
                    <option value="uts" {{ old('jenis', $ujian->jenis) == 'uts' ? 'selected' : '' }}>UTS</option>
                    <option value="uas" {{ old('jenis', $ujian->jenis) == 'uas' ? 'selected' : '' }}>UAS</option>
                    <option value="try_out" {{ old('jenis', $ujian->jenis) == 'try_out' ? 'selected' : '' }}>Try Out</option>
                </select>
                @error('jenis')
                    <p class="text-danger text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="mb-6">
            ...
            <textarea name="deskripsi" id="deskripsi" rows="3" 
                      ...
                      placeholder="Deskripsi ujian (opsional)">{{ old('deskripsi', $ujian->deskripsi) }}</textarea>
            @error('deskripsi')
                <p class="text-danger text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <input type="datetime-local" name="waktu_mulai" id="waktu_mulai" 
                       ...
                       value="{{ old('waktu_mulai', $ujian->waktu_mulai->format('Y-m-d\TH:i')) }}" required>
                @error('waktu_mulai')
                    <p class="text-danger text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" name="waktu_selesai" id="waktu_selesai" 
                       ...
                       value="{{ old('waktu_selesai', $ujian->waktu_selesai->format('Y-m-d\TH:i')) }}" required>
                @error('waktu_selesai')
                    <p class="text-danger text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="durasi" id="durasi" min="1" 
                       ...
                       placeholder="60" value="{{ old('durasi', $ujian->durasi) }}" required>
                @error('durasi')
                    <p class="text-danger text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="bg-secondary-100 rounded-xl p-4 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="acak_soal" value="1" class="form-checkbox" {{ old('acak_soal', $ujian->acak_soal) ? 'checked' : '' }}>
                    <span class="text-sm">Acak Urutan Soal</span>
                </label>
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="acak_opsi" value="1" class="form-checkbox" {{ old('acak_opsi', $ujian->acak_opsi) ? 'checked' : '' }}>
                    <span class="text-sm">Acak Urutan Opsi</span>
                </label>
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="tampil_nilai" value="1" class="form-checkbox" {{ old('tampil_nilai', $ujian->tampil_nilai) ? 'checked' : '' }}>
